...
- 教师输入与 AI 可见输出都必须通过 `msgSecCheck`；`SEC_CHECK` 未开启时正式调用会失败关闭。
...
`config.json` 已声明 `security.msgSecCheck` 云调用权限；部署后仍须在云函数权限页确认该权限已生效。依赖锁文件固定使用发布时的微信官方 `wx-server-sdk 4.0.2`，后续升级前必须复跑本目录测试与云端健康检查。
