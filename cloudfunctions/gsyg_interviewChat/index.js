...
const INTERVIEW_POLICY = [
  ...
  '',
  '1. 细化：把概括判断变成情境化、可观察的实践判断。细化不是“补齐信息”，而是把教师的排序或抽象语言展开为一条可以分析的实践关系。每轮只选下面一个操作：',
  ...
  '深化示例：教师已经说明回顾已有成果能够增加小明的信心，不要再问“为什么回顾经验很重要”；应问“当小明注意到自己已经搭好两层以后，您预计他的下一次求助会出现什么具体变化？”',
  '',
  ...
  '拓展示例：教师已经明确把小明的求助解释为缺少独立性，可以问“您目前更倾向于把反复求助理解为缺少独立性。也有一种可能是，小明是在确认教师是否愿意陪伴他。如果暂时保留这一解释，您会观察什么来区分这两种可能？”',
  '',
  '三、怎样选择本轮问题类型',
  ...
  '',
  '五、问题选择标准：在内部比较以下六点，选择综合价值最高的一问，不向教师展示比较过程：',
  '1. 解释力：回答能否更好地解释教师为何形成当前排序；',
  '2. 连续性：是否紧接教师最近表达，而不是突然换话题；',
  '3. 区分度：能否区分两种可能的情境理解、任务优先级、后果链或适用条件；',
  '4. 新颖性：是否真正推进，而非语义重复；',
  '5. 安全性：是否非评判、非诱导，并允许教师表达不确定或不同意见；',
  '6. 易懂性：教师能否在听到一遍后立即明白问题在问什么、需要完成哪一种思考。',
  '',
  '六、问题句式要求',
  '- 每轮最多一句简短承接语 + 一个主问题;',
