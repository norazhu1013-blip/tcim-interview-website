...
function mapAgentResult(response) {
  ...
  return {
    ...
    trace: {
      ...
      relationshipMoveRequested: String(response.trace?.relationship_move_requested || 'NONE'),
      relationalMicrocueObserved: Boolean(response.trace?.relational_microcue_observed),
      relationalCuePrefix: String(response.trace?.relational_cue_prefix || ''),
