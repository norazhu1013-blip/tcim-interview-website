...
- **2026-08-20 网页端改为正式账号密码登录**:网页不再用 CloudBase 匿名身份自动登录，改为研究者发放的 `username + password` 正式账号；姓名只保留为可修改的个人资料字段，绝不用于识别账号或授予管理员权限。`gsyg_webGateway` 会从 CloudBase `/auth/v1/user/me` 同时验证 UID 与非匿名账号证据，匿名 token 一律拒绝；会话版本升至 v2、`identityType='web_account'`，旧匿名 Cookie 自动失效。管理员仍只按新账号对应的 `web:<UID>` 在 `gsyg_teachers.isAdmin=true` 判定。上线前必须在 CloudBase 开启用户名密码登录、创建/确认 Nora 正式账号并把其新 UID 的 teacher 记录设为管理员；不得按姓名迁移权限。历史匿名数据保留，不自动合并到新账号。
