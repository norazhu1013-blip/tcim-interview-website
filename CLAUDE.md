...
- **2026-07-31 网页访谈生成超时与失败审计修复**:线上日志确认第三方模型曾在 21–22 秒成功生成，但 `gsyg_webGateway` 内部 `cloud.callFunction` 使用 SDK 默认 15 秒等待，导致网页误报“问题没有生成”。网关现对 `interviewChat` 显式使用 65 秒上游等待（其他动作仍为15秒），部署时网关函数自身超时须至少70秒；网页 `InterviewView` 另保留每次失败的时间、耗时、阶段、教师回答轮数和错误码到 `generationFailures[]`，即使教师随后主动结束也不清空历史。此次不因个别“啊？”增加提示词或结束语特判，访谈策略保持不变。
