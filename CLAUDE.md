...
- **登录:手机号授权按钮常驻(功能一直在),`-604101` 无权限只降级不挡登录。** 登录判断统一 `isLoggedIn`=`!!login_state.phoneAuthed`;守卫 `store.requireLogin()` 在所有非登录页 `onShow`(home/profile/interviewList/done)或 `onLoad`(exam/submit/score/review/interview)顶部 `if(!store.requireLogin())return;`,未登录 `reLaunch` 登录页。
