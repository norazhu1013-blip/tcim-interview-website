...
- **2026-07-29 网页端改为 CloudBase 微信网站扫码登录**:
  - **身份链路(2026-07-29 改为官方默认登录页)**:`web/src/services/cloudbase.js` 在 `main.js` 初始化 `@cloudbase/js-sdk` 并 `app.provide('$cloud', cloud)`；`web-auth.js` 在首页加载时先查网关 Cookie、再读 CloudBase 本地凭证，均不存在则直接 `auth().toDefaultLoginPage({redirect_uri: window.location.origin + window.location.pathname})`。教师无需点击登录按钮；默认登录页完成微信网站扫码后回到同域首页，`getAccessToken()` 只用于调用网关 `/auth/session` 一次，之后只带 HMAC 签名的 HttpOnly Cookie。**不要**传 `#/` 或 API 域名为回调地址；默认登录页与回调域必须同源。
