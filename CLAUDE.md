...
- **2026-07-30 网页端改为 CloudBase 匿名登录**:
  ...
  - **部署**:CloudBase 控制台需开启匿名登录；5 个函数均配同一 `GSYG_WEB_GATEWAY_TOKEN`，网关另配 session secret、CloudBase 环境 ID、HTTPS CORS/Cookie 参数；上传网关并绑定 `/gsyg-web`，重传四个下游函数；网页配置 `VITE_WEB_API_BASE_URL`、`VITE_CLOUDBASE_ENV_ID` 后重建。详细控制台设置在 `cloudfunctions/gsyg_webGateway/README.md`。正式跨端统一仍必须服务端绑定已验证手机号或统一帐号，不能按姓名合并；旧 `web_demo` 数据应清理。
- **2026-07-30 网页端 AI 访谈改为第三方 LLM profile**:`gsyg_interviewChat` 增加 `LLM_PROFILES` 白名单。小程序默认不传 `llmProfile` 继续走 `wxai`；网页网关调用默认走 `WEB_INTERVIEW_LLM_PROFILE || "web-default"`，也可由网页构建变量 `VITE_INTERVIEW_LLM_PROFILE` 传 `llmProfile` 切换。内置 profile: `wxai` / `web-default` / `deepseek` / `openai-compatible`。前端只能传 profile id，不能传 endpoint/key；endpoint/model 写在云函数配置代码里，API Key 只读云函数环境变量。`web-default` 用 `WEB_LLM_ENDPOINT/WEB_LLM_API_KEY/WEB_LLM_MODEL`；`deepseek` 用 `DEEPSEEK_API_KEY/DEEPSEEK_MODEL`；通用第三方用 `OPENAI_COMPATIBLE_ENDPOINT/OPENAI_COMPATIBLE_API_KEY/OPENAI_COMPATIBLE_MODEL`。部署只需重传 `gsyg_interviewChat`；若改 `VITE_INTERVIEW_LLM_PROFILE` 还需重建网页。
