...
- **2026-07-15 三项改动(据研究团队《AI访谈运行提示词 v4.0》+ 新题序程序 + 反馈问题)**:
  ...
  - **③ 计分/情境筛选参考程序更新为「新题序新赋分」**(`DOC/calculate_advisor_rpg_final_新题序新赋分.py`):研究团队把 Python 内部题号重排为**与小程序题号一致**(py Qn == mp Qn,MP_TO_PY 变恒等)。经校验为旧程序的**保值重排**——`SCORE_CSV`:`new[mpQ]===old[MP_TO_PY[mpQ]]` 全 240 处 0 不一致(赋分值未变,仅换列);`ABILITY_MAP` 标题/主/次能力在 MP_TO_PY 下 10/10 一致。且 mp `data/scoreTable.js` 与新程序赋分逐格 0/240 一致(mp 早已对齐)。**结论:mp 端行为无需改动**——`advisor_port.js` 刻意保留旧内部题号 + `gsyg_selectFinal` 的 MP_TO_PY(与 2026-07-09 决策一致),避免作废 `advisor_norms.js` 常模与三份对拍参考(verify_align/verify_norms_mode/verify_cf_pipeline 仍 45/45·0 diff,已复跑确认)。最终遴选结果与新程序在小程序题号下**完全等价**。`advisor_port.js` 头注已记录该等价性;若日后要落地恒等题号,需重生成 norms + 三份 ref_output 并重跑对拍。旧参考 `DOC/calculate_advisor_rpg_final(1).py` 保留(advisor_port 的 1:1 provenance)。
