...
- **2026-09-11 赋分口径恢复为研究团队《新10题赋分表 new.xlsx》**:逐格核对当前0817版与用户指定工作表「10题赋分表_修订版」，10题×24排列共240格中117格不同，涉及Q2/Q3/Q4/Q5/Q7/Q8/Q9/Q10。现以该工作表为唯一口径，恢复 `DOC/AI 测评 访谈及报告流程/000 10题赋分.xlsx`、小程序/网页共用 `scoreTable.js`、Node筛题端口、Python参考程序及45人参考输出；AI仍不参与评分。已有 `selection` 继续保留历史缓存，避免已完成或进行中的访谈题目被追溯改写；无既有遴选结果的新会话按本次指定表计分和筛题。过程常模只由日志行为指标组成，不因赋分变化重建。
- **2026-09-19 整体访谈生成链稳定性修复**:
  - **云端证据**:环境 `tcim-interview-d5g1hnd55c5310e51` 在 2026-09-18 的整体访谈调用中，多数模型轮次为约3—8秒，但出现44.191秒与72.192秒长尾；后者超过网页等待上限。旧实现先调用 `generateText`，失败或空结果后再重新调用 `streamText`，会把一次教师发送放大为两次串行模型请求。
  - **单次上游调用**:`gsyg_overallInterview` 每轮只发起一次 `streamText`；仅当当前SDK根本不提供流式接口时才使用 `generateText`，不再在同一轮失败后串行调用另一接口。流读取兼容 CloudBase `textStream/eventStream/dataStream` 与顶层异步迭代器，空流显式报错。
  - **网页恢复策略**:整体访谈网页为请求设置明确等待上限；只对12秒内快速失败的瞬时故障自动重试一次。已经等待较久后不再静默开启第二个完整模型请求，而是保留同一 `requestId`，提示教师手动重试，避免“长等—失败—再长等”。
  - **验证**:新增三类CloudBase流返回形态的自动测试；整体访谈AI测试3/3、网关/导入/烟雾测试8/8通过。
  - **腾讯云发布**:提交 `06ed962` 已于2026-09-19部署到环境 `tcim-interview-d5g1hnd55c5310e51` 的 `gsyg_overallInterview` 与 `gsyg_webGateway`，两者状态均为 `Deployment completed`；正式域名公开脚本已核对包含58秒请求上限与“仅12秒内瞬时失败静默补试一次”的规则，整体访谈页面返回200、无会话接口按预期返回401。
  - **依赖发布方式**:两个函数均改由腾讯云根据 `package-lock.json` 在线安装依赖，函数代码包不再携带本机 `node_modules/.bin` 符号链接和隐藏文件；这只改变打包方式，不改变生产环境变量、密钥、提示词、模型或访谈规则。
- **2026-09-19 整体访谈20题题库按本轮测验更新**:
  - 本轮上传数据共25题，整体访谈只使用前20道游戏支持与引导题；末5道教师情绪题由研究人员在上传前从结果表和过程表中筛除，不进入访谈输入。
  - 20题中实际测验题号 `XXXX0114` 为“小班停车场深蓝、浅蓝材料”情境，取代上一版整体访谈题库中的 `XXXX1027`“动物园建构”情境；题干与过程记录中实际出现的B选项以测验导出为准，导出未包含的A/C/D选项以研究者提供的《09.19_游戏支持与引导能力测题_20题》为准。
  - 题库版本升为 `TCIM-20题/2026-09-19`；`XXXX1027` 不再作为本轮20题的合法别名，避免把旧题误解释为本轮作答。
- **2026-09-19 整体访谈管理员上传入口显性化**:
  - 整体访谈首页直接显示“管理员上传资料”，普通教师的姓名检索和访谈入口保持不变。
  - 上传页继续用既有管理员会话保护；未登录时不显示文件选择框，而是打开研究数据工作台完成管理员登录。登录后回到上传页点击“我已登录，显示上传界面”，重新核验会话成功后才展示导入表单。
  - 这一调整只改善入口和登录后的返回流程，不放宽管理员权限，不上传或修改现有教师数据。
- **2026-09-19 整体访谈时长调整为12分钟**:
  - 新建的整体访谈会话截止时间由10分钟调整为12分钟，首页时长提示与模型访谈任务说明同步更新。
  - 已经创建的会话保留原有截止时间，不追溯改写历史记录；版本升为 `tcim-overall-interview/0.1.1`。
- **2026-09-19 整体访谈时长进一步调整为16分钟**:
  - 根据最新试访谈反馈，新建整体访谈会话由12分钟调整为16分钟；首页提示、模型任务说明与截止时间同步更新。
  - 既有会话继续保留创建时的截止时间；程序版本升为 `tcim-overall-interview/0.1.2`，提示版本升为 `tcim-overall-interview/2026-09-19-v3-16m`。
- **2026-09-20 整体访谈时长调整为25分钟**:
  - 新建整体访谈会话由16分钟调整为25分钟；首页提示、模型任务说明与截止时间同步更新。
  - 既有会话不追溯修改；程序版本升为 `tcim-overall-interview/0.1.3`，提示版本升为 `tcim-overall-interview/2026-09-20-v4-25m`。
  - 本次只调整访谈时长，不加入跨教师配额、主题硬上限或固定覆盖题数；后续选题改进以教师个人数据中的辨识度线索为核心，避免用硬性维度规则限制有价值的深入追问。
