...
# 按真实10题顺序修正后的能力类型映射。
# 004文档中的题号-情境简称与真实测验题号存在错位；这里保留其能力类型判断，
# 但映射到真实题号，确保最终导出的“第几题”和情境原文一致。
ABILITY_MAP = {
...
def method_note_rows(log_base, log_base_note):
    return [
        ["项目", "说明"],
        ["版本定位", "综合优化版：保留R/P/G/最终3题框架，同时加入IIV规范化指标作为量化底座。"],
